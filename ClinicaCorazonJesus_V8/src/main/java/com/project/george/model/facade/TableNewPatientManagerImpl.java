...
public class TableNewPatientManagerImpl implements TableNewPatientManager {
	...
	public List<TbNewPatientDTO> listAllPatientSpecificValues(String lastName,String dniValue) throws Exception {
		System.out.println("Inside listAllPatientSpecificValues");
		TbNewPatient beanPatient=new TbNewPatient();
		UtilMethods utilMethods=new UtilMethods();
		beanPatient.setLastNamePatient(lastName);
		beanPatient.setDni(dniValue);
		List<TbNewPatient> listAllPatient=custmTableNewPatient.listAllNewPatientSpecific(beanPatient);
		
		List<TbNewPatientDTO> newListPatient=new ArrayList<TbNewPatientDTO>();
		//--Add Reference Code History
		TbSystemParam beanSystemParam=tbSystemMan.getValueSystemParam(CommonUtil.SystemParam.NAME_PARAM_HISTORY_CLINIC);
		for(TbNewPatient beanPatientResult:listAllPatient){
			TbNewPatientDTO beanPatientDTO=new TbNewPatientDTO();
			beanPatientDTO=utilMethods.copyValuesNewPatientDTO(beanPatientResult, beanPatientDTO);
			beanPatientDTO.setCodeHistoryClinic(beanSystemParam.getValueParam()+beanPatientDTO.getCodeHistoryClinic());
			newListPatient.add(beanPatientDTO);
		}
		return newListPatient;
